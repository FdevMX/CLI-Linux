...
commands_list = ['cat', 'ls', 'echo', 'mkdir', 'pwd', 'cd', 'help', 'history', 'clear', 'cls']
...
def execute_command(parsed_command):
    ...
    try:
        if command == "cd":
            ...
            result = subprocess.run("Get-History" if os.name == "nt" else "history", shell=True, capture_output=True, text=True)
            return result.stdout if result.returncode == 0 else result.stderr
